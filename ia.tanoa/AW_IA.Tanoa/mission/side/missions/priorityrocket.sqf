...
//-------------------- 1. FIND POSITION
	...
	while {!_accepted} do {
		...
		while {(count _flatPos) < 2} do {
			_position = [[[getMarkerPos currentAO,5000]],["water","out"]] call BIS_fnc_randomPos;
			_flatPos = _position isFlatEmpty [5, 0, 0.2, 5, 0, false];
		};
...
//-------------------- 2. SPAWN OBJECTIVES
	...
	priorityObj1 = "RHS_BM21_MSV_01" createVehicle _flatPos1;
	waitUntil {!isNull priorityObj1};
	priorityObj1 setDir _PTdir;
	
	sleep 0.3;
	
	priorityObj2 = "RHS_BM21_MSV_01" createVehicle _flatPos2;
...
//-------------------- 3. SPAWN CREW
	...
	priorityGunner1 = ((units _priorityGroup) select 0);
	priorityGunner2 = ((units _priorityGroup) select 1);
	...
	((units _priorityGroup) select 0) assignAsGunner priorityObj1;
	((units _priorityGroup) select 0) moveInGunner priorityObj1;
	((units _priorityGroup) select 1) assignAsGunner priorityObj2;
